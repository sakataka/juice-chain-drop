...
import type { ProgressionStage } from "../core";
import { BGM_BASS, BGM_DRUMS, BGM_LOOP_BARS, BGM_MELODY, BGM_STAGE_BPMS, type BeatDuration, type BgmDrumHit, type BgmNote } from "./bgmComposition";
...
export class BgmPreview {
  ...
  constructor(volume: number) {
    ...
    for (const part of [this.melodyPart, this.bassPart, this.drumPart]) {
      part.loop = true;
  ...
  start(): void {
    ...
    this.started = true;
    Tone.Transport.bpm.value = BGM_STAGE_BPMS[this.stage];
  ...
  setStage(stage: ProgressionStage): void {
    this.stage = stage;
    Tone.Transport.bpm.value = BGM_STAGE_BPMS[stage];
  }
